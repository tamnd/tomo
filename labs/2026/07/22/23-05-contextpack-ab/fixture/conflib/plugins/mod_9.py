"""Plugin 9: a distractor loader with the same shape as the real one but a
...
def register_9(store, source):
    """Load plugin 9 data into the store from a file or module path."""
    if "/" in source:
        base = parse_file(source)
        for key, value in base.items():
            store[key] = value
        env = current_env(store)
        extra = f"{env.lower()}_9_{source}"
...
def describe_9():
    return "plugin 9 loader, unrelated to settings loading"
