"""Plugin 1: a distractor loader with the same shape as the real one but a
...
def register_1(store, source):
    """Load plugin 1 data into the store from a file or module path."""
    if "/" in source:
        base = parse_file(source)
        for key, value in base.items():
            store[key] = value
        env = current_env(store)
        extra = f"{env.lower()}_1_{source}"
...
def describe_1():
    return "plugin 1 loader, unrelated to settings loading"
