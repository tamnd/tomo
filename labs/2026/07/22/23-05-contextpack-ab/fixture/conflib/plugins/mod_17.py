"""Plugin 17: a distractor loader with the same shape as the real one but a
...
def register_17(store, source):
    """Load plugin 17 data into the store from a file or module path."""
    if "/" in source:
        base = parse_file(source)
        for key, value in base.items():
            store[key] = value
        env = current_env(store)
        extra = f"{env.lower()}_17_{source}"
...
def describe_17():
    return "plugin 17 loader, unrelated to settings loading"
