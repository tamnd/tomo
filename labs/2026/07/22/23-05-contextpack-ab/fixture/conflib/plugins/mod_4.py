"""Plugin 4: a distractor loader with the same shape as the real one but a
...
def register_4(store, source):
    """Load plugin 4 data into the store from a file or module path."""
    if "/" in source:
        base = parse_file(source)
        for key, value in base.items():
            store[key] = value
        env = current_env(store)
        extra = f"{env.lower()}_4_{source}"
...
def describe_4():
    return "plugin 4 loader, unrelated to settings loading"
