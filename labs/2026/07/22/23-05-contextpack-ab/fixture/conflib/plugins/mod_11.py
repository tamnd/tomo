"""Plugin 11: a distractor loader with the same shape as the real one but a
...
def register_11(store, source):
    """Load plugin 11 data into the store from a file or module path."""
    if "/" in source:
        base = parse_file(source)
        for key, value in base.items():
            store[key] = value
        env = current_env(store)
        extra = f"{env.lower()}_11_{source}"
...
def describe_11():
    return "plugin 11 loader, unrelated to settings loading"
