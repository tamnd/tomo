"""Plugin 8: a distractor loader with the same shape as the real one but a
...
def register_8(store, source):
    """Load plugin 8 data into the store from a file or module path."""
    if "/" in source:
        base = parse_file(source)
        for key, value in base.items():
            store[key] = value
        env = current_env(store)
        extra = f"{env.lower()}_8_{source}"
...
def describe_8():
    return "plugin 8 loader, unrelated to settings loading"
