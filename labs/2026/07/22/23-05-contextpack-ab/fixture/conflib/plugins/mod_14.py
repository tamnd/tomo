"""Plugin 14: a distractor loader with the same shape as the real one but a
...
def register_14(store, source):
    """Load plugin 14 data into the store from a file or module path."""
    if "/" in source:
        base = parse_file(source)
        for key, value in base.items():
            store[key] = value
        env = current_env(store)
        extra = f"{env.lower()}_14_{source}"
...
def describe_14():
    return "plugin 14 loader, unrelated to settings loading"
