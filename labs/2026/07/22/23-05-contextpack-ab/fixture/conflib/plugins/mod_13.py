"""Plugin 13: a distractor loader with the same shape as the real one but a
...
def register_13(store, source):
    """Load plugin 13 data into the store from a file or module path."""
    if "/" in source:
        base = parse_file(source)
        for key, value in base.items():
            store[key] = value
        env = current_env(store)
        extra = f"{env.lower()}_13_{source}"
...
def describe_13():
    return "plugin 13 loader, unrelated to settings loading"
