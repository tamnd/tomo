"""Plugin 5: a distractor loader with the same shape as the real one but a
...
def register_5(store, source):
    """Load plugin 5 data into the store from a file or module path."""
    if "/" in source:
        base = parse_file(source)
        for key, value in base.items():
            store[key] = value
        env = current_env(store)
        extra = f"{env.lower()}_5_{source}"
...
def describe_5():
    return "plugin 5 loader, unrelated to settings loading"
