"""Plugin 20: a distractor loader with the same shape as the real one but a
...
def register_20(store, source):
    """Load plugin 20 data into the store from a file or module path."""
    if "/" in source:
        base = parse_file(source)
        for key, value in base.items():
            store[key] = value
        env = current_env(store)
        extra = f"{env.lower()}_20_{source}"
...
def describe_20():
    return "plugin 20 loader, unrelated to settings loading"
