"""Plugin 21: a distractor loader with the same shape as the real one but a
...
def register_21(store, source):
    """Load plugin 21 data into the store from a file or module path."""
    if "/" in source:
        base = parse_file(source)
        for key, value in base.items():
            store[key] = value
        env = current_env(store)
        extra = f"{env.lower()}_21_{source}"
...
def describe_21():
    return "plugin 21 loader, unrelated to settings loading"
