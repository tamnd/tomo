"""Plugin 16: a distractor loader with the same shape as the real one but a
...
def register_16(store, source):
    """Load plugin 16 data into the store from a file or module path."""
    if "/" in source:
        base = parse_file(source)
        for key, value in base.items():
            store[key] = value
        env = current_env(store)
        extra = f"{env.lower()}_16_{source}"
...
def describe_16():
    return "plugin 16 loader, unrelated to settings loading"
