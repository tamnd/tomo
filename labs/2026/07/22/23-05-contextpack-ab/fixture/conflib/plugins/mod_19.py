"""Plugin 19: a distractor loader with the same shape as the real one but a
...
def register_19(store, source):
    """Load plugin 19 data into the store from a file or module path."""
    if "/" in source:
        base = parse_file(source)
        for key, value in base.items():
            store[key] = value
        env = current_env(store)
        extra = f"{env.lower()}_19_{source}"
...
def describe_19():
    return "plugin 19 loader, unrelated to settings loading"
