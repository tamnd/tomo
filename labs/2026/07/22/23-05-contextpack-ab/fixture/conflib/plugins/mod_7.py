"""Plugin 7: a distractor loader with the same shape as the real one but a
...
def register_7(store, source):
    """Load plugin 7 data into the store from a file or module path."""
    if "/" in source:
        base = parse_file(source)
        for key, value in base.items():
            store[key] = value
        env = current_env(store)
        extra = f"{env.lower()}_7_{source}"
...
def describe_7():
    return "plugin 7 loader, unrelated to settings loading"
