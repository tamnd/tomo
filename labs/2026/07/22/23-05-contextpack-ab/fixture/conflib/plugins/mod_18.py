"""Plugin 18: a distractor loader with the same shape as the real one but a
...
def register_18(store, source):
    """Load plugin 18 data into the store from a file or module path."""
    if "/" in source:
        base = parse_file(source)
        for key, value in base.items():
            store[key] = value
        env = current_env(store)
        extra = f"{env.lower()}_18_{source}"
...
def describe_18():
    return "plugin 18 loader, unrelated to settings loading"
