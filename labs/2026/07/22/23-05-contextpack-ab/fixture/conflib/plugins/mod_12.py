"""Plugin 12: a distractor loader with the same shape as the real one but a
...
def register_12(store, source):
    """Load plugin 12 data into the store from a file or module path."""
    if "/" in source:
        base = parse_file(source)
        for key, value in base.items():
            store[key] = value
        env = current_env(store)
        extra = f"{env.lower()}_12_{source}"
...
def describe_12():
    return "plugin 12 loader, unrelated to settings loading"
