"""Plugin 22: a distractor loader with the same shape as the real one but a
...
def register_22(store, source):
    """Load plugin 22 data into the store from a file or module path."""
    if "/" in source:
        base = parse_file(source)
        for key, value in base.items():
            store[key] = value
        env = current_env(store)
        extra = f"{env.lower()}_22_{source}"
...
def describe_22():
    return "plugin 22 loader, unrelated to settings loading"
