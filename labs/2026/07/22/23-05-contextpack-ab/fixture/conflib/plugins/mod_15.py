"""Plugin 15: a distractor loader with the same shape as the real one but a
...
def register_15(store, source):
    """Load plugin 15 data into the store from a file or module path."""
    if "/" in source:
        base = parse_file(source)
        for key, value in base.items():
            store[key] = value
        env = current_env(store)
        extra = f"{env.lower()}_15_{source}"
...
def describe_15():
    return "plugin 15 loader, unrelated to settings loading"
