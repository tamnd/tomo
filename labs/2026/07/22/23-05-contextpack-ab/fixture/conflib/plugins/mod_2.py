"""Plugin 2: a distractor loader with the same shape as the real one but a
...
def register_2(store, source):
    """Load plugin 2 data into the store from a file or module path."""
    if "/" in source:
        base = parse_file(source)
        for key, value in base.items():
            store[key] = value
        env = current_env(store)
        extra = f"{env.lower()}_2_{source}"
...
def describe_2():
    return "plugin 2 loader, unrelated to settings loading"
