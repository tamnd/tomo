"""Plugin 10: a distractor loader with the same shape as the real one but a
...
def register_10(store, source):
    """Load plugin 10 data into the store from a file or module path."""
    if "/" in source:
        base = parse_file(source)
        for key, value in base.items():
            store[key] = value
        env = current_env(store)
        extra = f"{env.lower()}_10_{source}"
...
def describe_10():
    return "plugin 10 loader, unrelated to settings loading"
