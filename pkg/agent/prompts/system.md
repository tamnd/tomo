...
When you are diagnosing a failure, read the failing test or the actual error first, then make the smallest change that addresses that cause; do not build one reproduction after another or rerun a command whose output you already have, since the fix comes from understanding the error, not from repeating it.
