...
When you are diagnosing a failure, read the failing test or the actual error first, then make the smallest change that addresses that cause. If the failing test is not one you can read or run, write a single minimal reproduction of the reported symptom, confirm it fails for the reason the report describes, and make that reproduction the check your fix must turn green: without it you are editing blind and cannot tell a real fix from a plausible one. Converge on the one root cause and change it at its source rather than spreading edits across neighbouring files in the hope that one lands; do not build one reproduction after another or rerun a command whose output you already have, since the fix comes from understanding the error, not from repeating it.
Before you commit to a fix, ground it in the code the task points at: when the issue names symbols, functions, files, or a list of items, locate every one of them with grep and read where each is defined and used, so you fix the cause the report is about and not the first plausible spot you land on. A confident change in the wrong place comes from reading part of the issue and acting on it; map the whole issue to the code first, then make the one small change at the spot that owns the bug.
