...
// CompactFromEnv reads the compaction knobs an operator can set to match the
// gate to the model in play. All default to zero, which leaves compaction off
// and the loop re-sending the full transcript exactly as before, so a plain
// build is unchanged. TOMO_COMPACT_TAIL sets the recent window kept verbatim,
// TOMO_COMPACT_BUDGET_TOKENS keys shedding to the model's context length (elide
// only past this estimate), and TOMO_COMPACT_MIN_BYTES sets the size a result
// must exceed to be elided. The env is the seam an A/B arm selects without a
// rebuild; a construction site copies the three into the Engine fields.
func CompactFromEnv() (tail, minBytes, budgetTokens int) {
	...
	return tail, minBytes, budgetTokens
}

// The re-sent-transcript problem, in the cx loop. Turn rebuilds each request as
// concat(history, turn), the whole conversation plus the running turn, so a file
// read or a build log captured twenty rounds ago goes back on the wire on every
// later call. Over a run that is O(n^2) in tokens: round k re-sends rounds 1..k,
// so the carried mass grows without bound and the late rounds dominate the bill.
// On a provider that does not cache the prefix that re-send is the largest share
// of a long run's cost, and even where the prefix is cached it is still billed
// at the hit rate.
//
// The fix is the cheap variant modern agents converge on: keep the recent window
// whole, and once the conversation grows past a budget keyed to the model's
// context length, shed the content of large, older tool results and leave a stub
// the model can re-fetch, since a read or a shell command is deterministic and
// cheap next to carrying its output forever. This mirrors the default engine's
// compaction so the two loops stay lean the same way while remaining independent.
...
//
// The gate is set two ways, both on the Engine so an operator can match it to
// the model in play, since a small-context model needs to shed sooner than a
// large one:
//
...
func (e *Engine) compactSend(msgs []provider.Message) []provider.Message {
	if e.CompactTail <= 0 && e.CompactBudgetTokens <= 0 {
	...
	minBytes := e.CompactMinBytes
	if minBytes <= 0 {
		minBytes = defaultCompactMinBytes
	}
	budgetBytes := e.CompactBudgetTokens * bytesPerToken
