...
Converge on the one root cause rather than editing across neighbouring files in the hope that one lands: a bug usually lives in one place, and a wide blast radius is the tell of a run reaching past the fix into code that was working.
...
- bash: run a shell command in your working directory. Use it to run the project's tests, build, or a quick reproduction, and to inspect the tree.
