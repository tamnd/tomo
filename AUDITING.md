...
| --- | --- | --- |
| `pkg/policy` | 297 | the fail-closed gate: allow/ask/deny, taint escalation, audit log |
...
a destructive tool. The append-only JSONL audit log is written here.

An attacker targets the decision table (find a class or rule combination that
returns allow when it should not) and the taint flag (get content ingested
without the session being marked tainted). Read `policy.go`, `guard.go`, and
`audit.go` together.
